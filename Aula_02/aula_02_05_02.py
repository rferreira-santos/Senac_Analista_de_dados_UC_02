...
tx_rec = ((rec / (roubo + furto)) * 100).apply(formatar)
total_tx_rec= (rec.sum()/(roubo.sum()+furto.sum()))*100
...
print('\n-- OBTENDO AS MÉTRICAS DOS ACAUMULADOS -- ')
print(f'Acumulado de roubos de veículos: {roubo.sum()}')
print(f'Acumulado de roubos de Furto: {furto.sum()}')
print(f'Acumulado de Recuperação de veículos: {rec.sum()}')
print(f'O Percentua do Acumulado de roubos e Furtos de veículos: {total_tx_rec:.2f}%')